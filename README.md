...
1. 两个账号优先读取公开 X 主页 HTML 发现新 ID；如果 GitHub 运行器被 X 返回 403，或页面没有可解析的帖子，则切换到免费的 FxTwitter v2 用户时间线。真正准备推送的新帖子仍会优先调用免费的 FxTwitter 详情中转源拿正文，解决 X 长文公开预览不完整的问题。
2. FxTwitter 也不可用时，只有 Serenity 才会继续使用 `aichainmap` 的公开 feed/页面作为备用；jukan 没有对应的 aichainmap 备份，不把两边的重复内容同时合并。
...
正式工作流开启 `REQUIRE_AI_ENRICHMENT=true` 和 `REQUIRE_X_FULL_TEXT=true`：智谱翻译或总结失败，或 X 长文详情暂时拿不到时，不发送该条、不写入已发送状态，下一分钟自动重试；现在按帖子逐条处理，单条模型安全拦截或接口故障不会阻塞同一轮其他完整帖子。智谱接口错误会在 Actions 日志中记录 HTTP 状态和 API 错误信息。
...
去重逻辑采用“先 claim、后发送”：这样优先避免并发任务造成重复推送；PushPlus 明确返回失败的帖子会进入 `retry_ids`，后续轮询自动重试。若运行器在发送成功后、写回状态前突然中断，仍无法用免费文件状态同时保证绝对不重复和绝不漏发。
